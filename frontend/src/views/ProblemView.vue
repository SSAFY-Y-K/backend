...
const isChildRoute = computed(() => route.name !== "problem");

const pageTitle = computed(() => {
	if (route.name === "problem-certification") return "자격증 문제";
	if (route.name === "problem-coding") return "코딩 문제";
	return "문제 풀기";
});
...
const goCertification = () => {
	router.push({ name: "problem-certification" });
};

const goCoding = () => {
	router.push({ name: "problem-coding" });
};
</script>
